...
def tab3_barh_store_subcat(chosen_store):

    grouped = df.merged[df.merged['Store_type']==chosen_store].groupby('Gender')['total_amt'].sum()

    fig = go.Figure(data=[go.Pie(labels=grouped.index,values=grouped.values)],layout=go.Layout(barmode='stack',margin={'t':20,}))
    return fig

@app.callback(Output('days','figure'),[Input('store_dropdown','value')])
def tab3_days(chosen_store):
    
    grouped = df.merged[df.merged['Store_type']==chosen_store].groupby('day_of_week')['total_amt'].sum()

    fig = go.Figure(data=[go.Bar(x=grouped.index.map({0:'Poniedziałek',1:'Wtorek',2:'Środa',3:'Czwartek',4:'Piątek',5:'Sobota',6:'Niedziela'}),y=grouped.values)],layout=go.Layout(title='Sprzedaż w dni tygodnia'))
    return fig
